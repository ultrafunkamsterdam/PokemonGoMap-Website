searchControl(action)'use strict';
...
var searchControlURI = 'https://vps.ultrafunk.nl/go/search_control';
function searchControl(action) {
  $.post('https://vps.ultrafunk.nl/go/?action=' + encodeURIComponent(action));
}
...
function loadRawData() {
  ...
  return $.ajax({
    url: 'http://vps.ultrafunk.nl/go/raw_data',
    type: 'GET',
    data: {
      'pokemon': loadPokemon,
...
function changeSearchLocation(lat, lng) {
  return $.post('https://vps.ultrafunk.nl/go/next_loc?lat=' + lat + '&lon=' + lng, {});
}
    ...
    if (navigator.geolocation && Store.get('geoLocate')) {
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        if (getPointDistance(marker.getPosition(), new google.maps.LatLng(lat, lon)) > 40) {
          $.post('https://vps.ultrafunk.nl/go/next_loc?lat=' + lat + '&lon=' + lon).done(function () {
