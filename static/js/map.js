...
var searchControlURI = mapsURL +'/search_control';
...
function changeSearchLocation (lat, lng) {
  return $.post(mapsURL + '/next_loc?lat=' + lat + '&lon=' + lng, {})
}
    ...
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      if (Store.get('geoLocate')) {
        ...
        if ((typeof searchMarker !== 'undefined') && (getPointDistance(searchMarker.getPosition(), center) > 40)) {
          $.post(mapsURL + '/next_loc?lat=' + lat + '&lon=' + lng).done(function () {
            map.panTo(center)
...
$(function () {
  ...
  $.getJSON('static/data/searchmarkerstyle.json').done(function (data) {
...
$(function () {
  ...
  $.getJSON('static/data/moves.json').done(function (data) {
    moves = data
  })
